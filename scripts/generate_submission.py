...
FILE_PATH = Path(__file__).parent.parent / "data" / "candidates.jsonl"
...
with open("submission.csv", "w", newline="", encoding="utf-8") as file:
    ...
    for rank, row in enumerate(scores[:100], start=1):

        reasoning = (
            f"{row[2]} with {row[3]:.1f} years experience, "
            f"{row[4]} JD skill matches, "
            f"GitHub activity score {row[5]}, "
            f"recruiter response rate {row[6]}, "
            f"and {row[7]} day notice period."
        )
